...
ifneq ($(USE_MINIMAL_FLAGS),0)
    ...
    ifeq ($(NO_FATAL_WARNINGS),1)
        ...
        BUILD_WARNING_FLAGS = $(WALL_FLAG) $(WEXTRA_FLAG)
        TEST_WARNING_FLAGS = $(BUILD_WARNING_FLAGS)
    else ifeq ($(NO_FATAL_TEST_WARNINGS),1)
        # No fatal warnings only when building tests.
        BUILD_WARNING_FLAGS = $(WALL_FLAG) $(WEXTRA_FLAG) $(WERROR_FLAG)
        TEST_WARNING_FLAGS = $(WALL_FLAG) $(WEXTRA_FLAG)
    else
        BUILD_WARNING_FLAGS = $(WALL_FLAG) $(WEXTRA_FLAG) $(WERROR_FLAG)
        TEST_WARNING_FLAGS = $(BUILD_WARNING_FLAGS)
    endif
